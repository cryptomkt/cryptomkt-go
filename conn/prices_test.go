...
func TestGetAllPrices(t *testing.T) {
	...
	for i := 0; i < 100; i++ {
		...
		switch numArgs {
		case 0:
			if price, err := client.GetPrices(args.Market("ETHCLP"), args.Timeframe("10080")); err != nil {
				t.Errorf("Prices with zero optional args failed, %s", err)
		...
		case 1:
			var randomIndex int = rand.Intn(2)
			if price, err := client.GetPrices(args.Market("ETHCLP"), args.Timeframe("10080"), optional[randomIndex]); err != nil {
				t.Errorf("Prices with one optional argument failed, %s", err)
				_, err2 := price.GetAllPrices()
				if err2 != nil {
		...
		case 2:
			if price, err := client.GetPrices(args.Market("ETHCLP"), args.Timeframe("10080"), optional[0], optional[1]); err != nil {
				t.Errorf("Prices with 2 optional args failed, %s", err)
				_, err2 := price.GetAllPrices()
				if err2 != nil {
